...
function buildOtpEmailHtml(otp: string, title?: string, description?: string): string {
  const heading = title || "Mã xác thực OTP";
  const desc =
    description ||
    "Bạn đang đăng ký tài khoản ChatSF. Vui lòng dùng mã OTP dưới đây để hoàn tất.";
  return `
<div style="background:#0D0F12;padding:32px 16px;font-family:'Inter',Arial,Helvetica,sans-serif;">
  <div style="max-width:560px;margin:0 auto;background:#111418;border:1px solid #2A2E35;border-radius:16px;overflow:hidden;">
    ...
    <div style="padding:24px;">
      <div style="color:#E6EEF8;font-size:18px;font-weight:600;margin-bottom:8px;">${heading}</div>
      ...
      <div style="color:#8C96A8;font-size:13px;line-height:1.6;">
        Mã có hiệu lực trong <b style="color:#E6EEF8;">10 phút</b>. Nếu không phải bạn thực hiện, hãy bỏ qua email này.
      </div>
    </div>
    <div style="padding:16px 24px;border-top:1px solid #2A2E35;background:#111418;color:#8C96A8;font-size:12px;">
      © ${new Date().getFullYear()} ChatSF. Bảo mật & an toàn là ưu tiên hàng đầu.
    </div>
  </div>
</div>
  `;
...
export async function sendOtpEmail(input: SendOtpEmailInput): Promise<void> {
  ...
  const html = buildOtpEmailHtml(input.otp, input.title, input.description);

  await transporter.sendMail({
    from: `ChatSF <${defaultSender}>`,
    to: input.to,
    subject: input.title ? `[ChatSF] ${input.title}` : "[ChatSF] Mã OTP",
    html,
    attachments: [
      {
        filename: "logo.png",
